...
open class FolioReaderCenter: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func configureNavBarButtons() {
        ...
        if (self.readerConfig.enableClose) {
            let close = UIBarButtonItem(image: closeIcon, style: .plain, target: self, action:#selector(closeReader(_:)))
            leftBarIcons.append(close)
        }
        
        if (self.readerConfig.enableMenu) {
            let menu = UIBarButtonItem(image: tocIcon, style: .plain, target: self, action:#selector(presentChapterList(_:)))
            leftBarIcons.append(menu)
        }
    ...
    //Hung added
    func checkBookmark() {
        guard let page = currentPage?.pageNumber else {
            return
        }
        
        if HMBookmarkManager.sharedInstance.exisedBookmarkAtPage(page) {
            bookmarkButton.image = UIImage(readerImageNamed: "icon-navbar-bookmark")?.ignoreSystemTint(withConfiguration: self.readerConfig)
        } else {
            bookmarkButton.image = UIImage(readerImageNamed: "icon-navbar-no-bookmark")?.ignoreSystemTint(withConfiguration: self.readerConfig)
        }
    }
    //--
    ...
    func bookmarkPage() {
        ...
        if HMBookmarkManager.sharedInstance.exisedBookmarkAtPage(page) {
            if HMBookmarkManager.sharedInstance.removeBookmarkAtPage(page) {
                checkBookmark()
            }
        } else {
            let bookmark = HMBookmark()
            bookmark.title = getCurrentChapterName() ?? ""
            bookmark.page = currentPage?.pageNumber
            if HMBookmarkManager.sharedInstance.add(bookmark: bookmark) {
                checkBookmark()
            }
        }
    }
    ...
    func getCurrentChapterName() -> String? {
        return getCurrentChapterName(page: self.currentPageNumber)
    }
    
    func getCurrentChapterName(page: Int) -> String? {
        if (page < 0) {
            return NSLocalizedString("No title", comment: "")
        }
        
        var mainTitle: String?
        var backupTitle: String?
        for item in self.book.flatTableOfContents {
            guard let reference = self.book.spine.spineReferences[safe: (page - 1)],
                let resource = item.resource,
            ...
            for tocReference in item.children {
                if let tocResource = tocReference.resource, tocResource == reference.resource {
                    backupTitle = title
                }
            }
        }
        
        if mainTitle != nil || backupTitle != nil {
            return mainTitle != nil ? mainTitle : backupTitle
        } else {
            return getCurrentChapterName(page:page - 1)
        }
    }
    ...
    open func changePageWith(page: Int, animated: Bool = false, completion: (() -> Void)? = nil) {
        if page > 0 && page-1 < totalPages {
            ...
            changePageWith(indexPath: indexPath, animated: animated, completion: {[weak self] () -> Void in
                self?.updateCurrentPage {
                    completion?()
                }
            })
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        var isCollectionScrollView = (scrollView is UICollectionView)
    ...
    func presentChapterList(_ sender: UIBarButtonItem) {
        ...
        let bookmark = HMBookmarkList(reader: folioReader, config: readerConfig)
        ...
        pageController.viewControllerTwo = bookmark
        pageController.viewControllerThree = highlight
        pageController.segmentedControlItems = [readerConfig.localizedContentsTitle, readerConfig.localizedBookmarksTitle, readerConfig.localizedHighlightsTitle]
